...
export async function POST(request: Request) {
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<!doctype html><html lang="de"><body style="margin:0;background:#f3f5f4;font-family:Arial,sans-serif;color:#172033;"><table role="presentation" width="100%"><tr><td align="center" style="padding:32px 12px;"><table role="presentation" width="100%" style="max-width:620px;background:#fff;border-top:7px solid #00a859;border-radius:10px;"><tr><td style="padding:30px;"><h1 style="margin:0 0 22px;font-size:24px;">Neue Registrierung</h1><p>Ein neues Konto wartet auf deine Prüfung.</p><table role="presentation" width="100%" style="margin:22px 0;background:#f5f7f6;border-collapse:collapse;"><tr><td style="padding:12px;color:#64748b;">Name</td><td style="padding:12px;font-weight:bold;">${escapeHtml(String(fullName))}</td></tr><tr><td style="padding:12px;color:#64748b;">E-Mail</td><td style="padding:12px;font-weight:bold;">${escapeHtml(user.email)}</td></tr></table><a href="${escapeHtml(approvalUrl)}" style="display:inline-block;padding:13px 20px;background:#172033;color:#fff;text-decoration:none;border-radius:7px;font-weight:bold;">Registrierung prüfen</a><p style="margin:22px 0 0;color:#64748b;font-size:13px;">Der Link ist 48 Stunden gültig. Die Freigabe erfolgt erst nach deiner Bestätigung auf der nächsten Seite.</p></td></tr></table></td></tr></table></body></html>`
    })
